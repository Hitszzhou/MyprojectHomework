...
void input_information(P peo[10]);//输入用户信息
...
void quick_sort_by_reverse_name(P peo[],int L,int R);//用快速排序,排序用户名
...
int main()
{
	...
	{
		menu();
		while ((ret = scanf("%d", &input) )< 1||(sum_of_peo<1&&(input!=1)))//判断输入是否合法,不合法则提醒用户重新输入
		{
		...
		case 1:
			...
			break;
		case 2:
			quick_sort_by_reverse_name(peoplearr,0,sum_of_peo-1);//按姓名排序用户
...
//函数功能:按名字逆序排序 入口:结构体数组的名字,左端点,右端点
...
{
	quick_sort_by_reverse_name(peo,0,sum_of_peo-1
                            );//先把名字逆序排序,再二分查找
